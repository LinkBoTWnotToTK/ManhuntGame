...
export function DustParticles({ count = 200 }: { count?: number }) {
  ...
  // Cut count in half for performance
  const actualCount = Math.floor(count / 2);
  ...
  const positions = useMemo(() => {
    ...
    return arr;
  }, [actualCount, selectedMap]);

  const speeds = useMemo(() => Array.from({ length: actualCount }, () => 0.02 + Math.random() * 0.05), [actualCount]);
  ...
  return (
    <points ref={ref}>
      <bufferGeometry>
        <bufferAttribute attach="attributes-position" array={positions} count={actualCount} itemSize={3} />
      </bufferGeometry>
