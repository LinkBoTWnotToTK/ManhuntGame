...
const STAMINA_DRAIN = 25; // per second
...
export default function Player() {
  ...
  const { role, isPlaying, escapeOpen, setEscaped, gameOver, stamina, useStamina, regenStamina } = useGame();
  ...
  useFrame((_, delta) => {
    ...
    if (isSprinting) {
      useStamina(STAMINA_DRAIN * delta);
    } else {
      regenStamina(STAMINA_REGEN * delta);
    }
    ...
    if (!blocked) {
      ...
      if (!bX) camera.position.x = slideX.x;
      ...
      if (!bZ) camera.position.z = slideZ.z;
    }
