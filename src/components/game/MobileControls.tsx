...
export default function MobileControls() {
  ...
  // Camera touch (right side of screen)
  useEffect(() => {
    ...
    const handleTouchStart = (e: TouchEvent) => {
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (touch.clientX > window.innerWidth * 0.5 && cameraTouchId.current === null) {
          // Make sure it's not on a button
    ...
    const handleTouchMove = (e: TouchEvent) => {
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (touch.identifier === cameraTouchId.current) {
          mobileCameraDelta.x = (touch.clientX - cameraLastPos.current.x) * 0.005;
          mobileCameraDelta.y = (touch.clientY - cameraLastPos.current.y) * 0.005;
          cameraLastPos.current = { x: touch.clientX, y: touch.clientY };
        }
      }
    };
  ...
  if (!isMobile || !isPlaying || gameOver) return null;
  ...
  return (
    <div className="fixed inset-0 z-[100] pointer-events-none" style={{ touchAction: "none" }}>
      ...
      {/* Weapon selector - top right (runner only) */}
      {role === "runner" && (
