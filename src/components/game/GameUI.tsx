...
export default function GameUI() {
  ...
  const {
    ...
    isPlaying, escapeOpen, stamina, maxStamina, selectRole, startGame, resetGame
  } = useGame();
        ...
        <>
          ...
          <div className="fixed inset-0 pointer-events-none z-50 flex items-center justify-center">
            <div className="w-8 h-8 relative">
              <div className="absolute top-1/2 left-0 w-2.5 h-0.5 bg-white/60 rounded-full" />
              <div className="absolute top-1/2 right-0 w-2.5 h-0.5 bg-white/60 rounded-full" />
              <div className="absolute left-1/2 top-0 h-2.5 w-0.5 bg-white/60 rounded-full" />
              <div className="absolute left-1/2 bottom-0 h-2.5 w-0.5 bg-white/60 rounded-full" />
                ...
                <div>
                  {role === "hunter" ? (
                    <>
                      ...
                      <div className="text-white/40 text-[10px] uppercase tracking-[0.2em]">Runners Tagged</div>
                    </>
                  ) : (
                    <>
      ...
      {gameOver && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-lg">
          <div className="text-center space-y-6 animate-scale-in">
            ...
            <p className="text-2xl text-white/60 font-mono tabular-nums">
              {formatTime(elapsedTime)}
            </p>
            {role === "hunter" && gameResult === "win" && (
              <p className="text-white/40">All {totalNPCs} runners tagged!</p>
            )}
            {role === "hunter" && gameResult === "lose" && (
              <p className="text-white/40">Runners escaped! Try again.</p>
            )}
            {role === "runner" && gameResult === "win" && (
              <p className="text-green-400/60">You escaped! 🎉</p>
            )}
            {role === "runner" && gameResult === "lose" && (
              <p className="text-red-400/60">Caught by a hunter!</p>
            )}
            <button
      ...
      {!isLocked && !gameOver && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/90 backdrop-blur-sm">
          <div className="text-center space-y-8 animate-fade-in max-w-xl">
            ...
            <p className="text-sm text-white/30 uppercase tracking-[0.3em]">First Person</p>
                ...
                <div className="flex gap-5 justify-center">
                  ...
                  >
                    ...
                    <div className="text-xs text-white/40 leading-relaxed">
                      Tag all 7 runners<br />before they escape
                    </div>
                    <div className="text-[10px] text-red-400/50 uppercase tracking-wider">Hard Mode</div>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <div className="text-[10px] text-blue-400/50 uppercase tracking-wider">Survival Mode</div>
                  </button>
                </div>
              </>
            ) : (
              <div className="space-y-5">
                <div className="bg-white/5 rounded-2xl p-5 border border-white/10 max-w-sm mx-auto">
                  <p className="text-white/70 leading-relaxed">
                    {role === "hunter"
                      ? "Hunt down all 7 runners scattered across the map. They're fast and tricky — use sprint wisely!"
                      : "Evade 3 hunters for 60 seconds. When the portal opens, sprint to it to escape!"}
                  </p>
                </div>
                <p className="text-white/90 text-lg font-bold">Click to start</p>
                <div className="flex gap-5 justify-center text-white/40 text-xs">
